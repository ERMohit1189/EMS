...
export default function MonthlyAttendance() {
    ...
    const isAllowed = employeeRole === 'admin' || employeeRole === 'user';
    if (!isAllowed) {
      return (
  ...
  const [loadingEmployees, setLoadingEmployees] = useState(false); // Loading state for employee dropdown
  ...
  const fetchTeams = async () => {
    try {
      ...
      // For superadmin: fetch all teams
      // For admin: try to fetch reporting teams first, if none found then fetch all teams
      if (isSuperAdmin) {
        ...
        // First try to fetch teams where user is reporting person
        console.log('[MonthlyAttendance] Admin - fetching reporting teams first');
        ...
        if (reportingResponse.ok) {
          ...
          if (Array.isArray(reportingTeams) && reportingTeams.length > 0) {
            // User is a reporting person for some teams, show only those
            console.log('[MonthlyAttendance] Using reporting teams only');
            setTeams(reportingTeams);
          } else {
            // User is not a reporting person, fetch all teams
            console.log('[MonthlyAttendance] No reporting teams found, fetching all teams');
            const allResponse = await fetch(`${getApiBaseUrl()}/api/teams`, {
              credentials: 'include',
            });
            if (allResponse.ok) {
              const allTeams = await allResponse.json();
              console.log('[MonthlyAttendance] All teams received:', allTeams);
      ...
      if (response.ok) {
        ...
        if (data?.attendanceData) {
          ...
          Object.keys(mergedDataRaw).forEach((k) => {
            ...
            mergedData[dayKey] = { status, ...(rec.leaveType ? { leaveType: rec.leaveType } : {}) } as any;
          });

          // Ensure Sundays and holidays remain as holidays
          for (let day = 1; day <= daysInMonth; day++) {
            if (isSunday(day) || holidayMap[day]) {
              mergedData[day] = { status: 'holiday' };
            }
          }
              ...
              return (
                <div key={day} className="relative">
                  <button
                    ...
                    disabled={isHoliday || isSubmitted}
                    ...
                    `}
                  >
                    <div className="text-[11px] font-semibold text-white leading-tight">{day}</div>
                    {status && (
