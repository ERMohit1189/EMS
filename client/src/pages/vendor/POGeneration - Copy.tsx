...
export default function POGeneration() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // Debug logging
        console.log('[POGeneration][DEBUG] vendorId from localStorage:', localStorage.getItem('vendorId'), 'normalizedStoredVendorId used:', normalizedStoredVendorId, 'vendorId state:', vendorId);
        refreshedSites.forEach((site: any) => {
          console.log('[POGeneration][DEBUG] site.id:', site.id, 'site.vendorId:', site.vendorId, 'type:', typeof site.vendorId);
        });
      } catch (error) {
  ...
  const exportPOToPDF = async (poId: string, poNumber: string) => {
    try {
      ...
      // Create HTML content matching the print page design
      const subtotal = lines && lines.length > 0
        ...
        <head>
          <style>
            * { box-sizing: border-box; }
            body { font-family: Arial, sans-serif; margin: 0; padding: 48px; background: white; }
            ...
            .company-info { font-size: 11px; margin-bottom: 20px; line-height: 1.6; }
          </style>
        </head>
        <body>
          <div class="po-header">
            <div class="po-company">
              ...
              <div class="po-subtitle">${exportHeaderSettings.companyName || 'Company Name'}</div>
            </div>
            <div class="po-info-box">
              <div class="po-info-row">
                <span class="po-info-label">PO No.:</span>
                <span class="po-info-value">${po.poNumber}</span>
              </div>
              <div class="po-info-row">
                <span class="po-info-label">PO Date:</span>
                <span class="po-info-value">${po.poDate}</span>
              </div>
              <div class="po-info-row">
                <span class="po-info-label">Due Date:</span>
                <span class="po-info-value">${po.dueDate}</span>
              </div>
            </div>
          </div>

          <div class="company-info">
            ${exportHeaderSettings.companyName ? `<div><strong>${exportHeaderSettings.companyName}</strong></div>` : ''}
            ${exportHeaderSettings.address ? `<div>${exportHeaderSettings.address}</div>` : ''}
            ${exportHeaderSettings.contactPhone ? `<div>Phone: ${exportHeaderSettings.contactPhone}</div>` : ''}
            ${exportHeaderSettings.website ? `<div>Website: ${exportHeaderSettings.website}</div>` : ''}
            ${exportHeaderSettings.gstin ? `<div>GSTIN: ${exportHeaderSettings.gstin}</div>` : ''}
          </div>
          ...
          <div class="party-info">
            <div class="party-section">
              ...
              ${vendor?.city ? `<div>${vendor.city}, ${vendor.state} ${vendor.pincode}</div>` : ''}
              ${vendor?.gstin ? `<div>GSTIN: ${vendor.gstin}</div>` : ''}
              ${vendor?.phone ? `<div>Phone: ${vendor.phone}</div>` : ''}
            </div>
            <div class="party-section">
              <div class="section-header">Sites Included</div>
              ${lines && lines.length > 0 ? `
                <div style="font-size: 10px;">
                  <div style="font-weight: bold; margin-bottom: 8px;">${lines.length} Site(s)</div>
                  <ul style="margin: 0; padding-left: 16px;">
                    ${lines.map(line => `<li style="margin-bottom: 4px;">${line.siteHopAB || line.siteId}${line.sitePlanId ? ` (${line.sitePlanId})` : ''}</li>`).join('')}
                  </ul>
                </div>
              ` : `
                <div class="party-name">${vendor?.name || '[Site Name]'}</div>
          ...
          <table class="items-table">
            ...
            <tbody>
              ${lines && lines.length > 0 ? lines.map(line => `
                <tr>
                  <td>
                    <div style="font-weight: bold;">${line.siteHopAB || line.siteId}</div>
                    <div style="font-size: 10px; color: #666;">${line.description || 'Installation and commissioning'}</div>
                  </td>
                  <td class="text-right">${line.quantity}</td>
                  <td class="text-right">Rs. ${Number(line.unitPrice).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  <td class="text-right">Rs. ${Number(line.totalAmount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              `).join('') : `
                <tr>
                  <td>${po.description}</td>
                  <td class="text-right">${po.quantity}</td>
                  <td class="text-right">Rs. ${Number(po.unitPrice).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  <td class="text-right">Rs. ${subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              `}
            </tbody>
          </table>
          ...
          <div class="po-footer">
            ...
            <p style="margin-top: 8px;">Status: <strong>${po.status}</strong></p>
          </div>
        </body>
        </html>
      `;
      ...
      // Note: GST amounts will be calculated after we know the subtotal from all lines
      ...
      // Site right column - Show all sites included
      let sy = y;
      pdf.setFont('Arial', 'bold');
      pdf.setFontSize(10);

      if (lines.length > 0) {
        pdf.text(`Sites Included (${lines.length})`, 105, sy);
        sy += 4;

        pdf.setFont('Arial', 'normal');
        pdf.setFontSize(8);
        for (const line of lines.slice(0, 6)) { // Show max 6 sites in header
          const siteName = line.siteHopAB || line.siteId;
          pdf.text(`• ${siteName}`, 105, sy);
          sy += 3;
        }
        if (lines.length > 6) {
          pdf.text(`... and ${lines.length - 6} more`, 105, sy);
        }
      } else {
      ...
      // ITEM DATA ROWS - Display all lines
      pdf.setFont('Arial', 'normal');
      pdf.setTextColor(0, 0, 0);
      pdf.setDrawColor(221, 221, 221);
      pdf.setFontSize(9);
      ...
      const itemsToDisplay = lines.length > 0 ? lines : [{
        description: po.description,
        quantity: po.quantity || 1,
        unitPrice: po.unitPrice || 0,
        totalAmount: po.totalAmount || 0,
        siteHopAB: po.siteName || po.siteId
      }];

      for (const item of itemsToDisplay) {
        const desc = String(item.siteHopAB || item.description || '').substring(0, 50);
        const qty = String(item.quantity || 1);
        const rate = Number(item.unitPrice || 0).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
        const itemTotal = Number(item.totalAmount || 0);
        const itemDisplay = itemTotal.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 });

        subtotalAmount += itemTotal;

        // Check if we need a new page (leave room for totals at bottom)
        if (y > 250) {
          // Add new page
          pdf.addPage();
          y = 20;
        ...
        // Description value
        pdf.rect(col1X, y, 80, 7);
        pdf.text(desc, col1X + 2, y + 3.5);

        // Quantity value
        pdf.rect(col2X, y, 30, 7);
        pdf.text(qty, col2X + 12, y + 3.5);

        // Unit Price value
        pdf.rect(col3X, y, 25, 7);
        pdf.text(`Rs. ${rate}`, col3X + 1, y + 3.5);

        // Amount value
        pdf.rect(col4X, y, 30, 7);
        pdf.text(`Rs. ${itemDisplay}`, col4X + 2, y + 3.5);
      ...
      const subtotalDisplay = subtotalAmount.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
      pdf.text('Subtotal:', tX, y);
      pdf.text(`Rs. ${subtotalDisplay}`, col4X + 2, y);
      y += 6;

      // Calculate GST amounts based on subtotal and PO settings
      if (po.gstApply) {
        if (po.gstType === 'igst') {
          igstAmt = Math.round(subtotalAmount * 0.18 * 100) / 100;
        } else if (po.gstType === 'cgstsgst') {
          cgstAmt = Math.round(subtotalAmount * 0.09 * 100) / 100;
          sgstAmt = Math.round(subtotalAmount * 0.09 * 100) / 100;
        }
      }
      ...
      const finalTotal = subtotalAmount + igstAmt + cgstAmt + sgstAmt;
