    ...
    importedData.forEach((row, idx) => {
      try {
        if (importType === 'site') {
          ...
          const siteData = {
            siteId: row['PLAN ID'] || `SITE-${idx}`,
            vendorId: vendors[0]?.id || '1',
            sno: Number(row['S.No.']) || undefined,
            circle: row['Circle'],
            planId: row['PLAN ID'] || '',
            nominalAop: row['Nominal Aop'],
            hopType: row['HOP TYPE'],
            hopAB: row['HOP A-B'],
            hopBA: row['HOP B-A'],
            district: row['District'],
            project: row['PROJECT'],
            siteAAntDia: row['SITE A ANT DIA'],
            siteBAntDia: row['SITE B ANT DIA'],
            maxAntSize: row['Max Ant Size'],
            siteAName: row['SITE A NAME'],
            tocoVendorA: row['TOCO VENDOR A'],
            tocoIdA: row['TOCO ID A'],
            siteBName: row['SITE B NAME'],
            tocoVendorB: row['TOCO VENDOR B'],
            tocoIdB: row['TOCO ID B'],
            mediaAvailabilityStatus: row['MEDIA AVAILABILITY STATUS'],
            srNoSiteA: row['SR NO SITE A'],
            srDateSiteA: row['SR DATE SITE A'],
            srNoSiteB: row['SR NO SITE B'],
            srDateSiteB: row['SR DATE SITE B'],
            hopSrDate: row['HOP SR DATE'],
            spDateSiteA: row['SP DATE SITE A'],
            spDateSiteB: row['SP DATE SITE B'],
            hopSpDate: row['HOP SP DATE'],
            soReleasedDateSiteA: row['SO RELEASED DATE SITE A'],
            soReleasedDateSiteB: row['SO RELEASED DATE SITE B'],
            hopSoDate: row['HOP SO DATE'],
            rfaiOfferedDateSiteA: row['RFAI OFFERED DATE SITE A'],
            rfaiOfferedDateSiteB: row['RFAI OFFERED DATE SITE B'],
            actualHopRfaiOfferedDate: row['ACTUAL HOP RFAI OFFERED DATE'],
            partnerName: row['PARTNER NAME'],
            rfaiSurveyCompletionDate: row['RFAI SURVEY COMPLETION DATE'],
            moNumberSiteA: row['MO NUMBER SITE A'],
            materialTypeSiteA: row['MATERIAL TYPE SITE A(SRN, FRESH, FRESH+SRN)'],
            moDateSiteA: row['MO DATE SITE A'],
            moNumberSiteB: row['MO NUMBER SITE B'],
            materialTypeSiteB: row['MATERIAL TYPE SITE B(SRN, FRESH, FRESH+SRN)'],
            moDateSiteB: row['MO DATE SITE B'],
            srnRmoNumber: row['SRN/RMO NUMBER'],
            srnRmoDate: row['SRN/RMO DATE'],
            hopMoDate: row['HOP MO DATE'],
            hopMaterialDispatchDate: row['HOP MATERIAL DISPATCH DATE'],
            hopMaterialDeliveryDate: row['HOP MATERIAL DELIVERY DATE'],
            materialDeliveryStatus: row['MATERIAL DELIVERY STATUS'],
            siteAInstallationDate: row['SITE A INSTALLATION DATE'],
            ptwNumberSiteA: row['PTW Number(Site A)'],
            ptwStatusA: row['PTW Status A'],
            siteBInstallationDate: row['SITE B INSTALLATION DATE'],
            ptwNumberSiteB: row['PTW Number(Site B)'],
            ptwStatusB: row['PTW Status B'],
            hopIcDate: row['HOP I&C DATE'],
            alignmentDate: row['Alignment Date'],
            hopInstallationRemarks: row['HOP Installation Remarks'],
            visibleInNms: row['VISIBLE IN NMS'],
            nmsVisibleDate: row['NMS VISIBLE DATE'],
            softAtOfferDate: row['SOFT AT OFFER DATE'],
            softAtAcceptanceDate: row['SOFT AT ACCEPTANCE DATE'],
            softAtStatus: row['SOFT-AT STATUS'],
            phyAtOfferDate: row['PHY-AT OFFER DATE'],
            phyAtAcceptanceDate: row['PHY-AT ACCEPTANCE DATE'],
            phyAtStatus: row['PHY-AT STATUS'],
            bothAtStatus: row['BOTH AT STATUS'],
            priIssueCategory: row['PRI ISSUE CATEGORY'],
            priSiteId: row['PRI SITE ID'],
            priOpenDate: row['PRI OPEN DATE'],
            priCloseDate: row['PRI CLOSE DATE'],
            priHistory: row['PRI HISTORY'],
            rfiSurveyAllocationDate: row['RFI Survey Allocation Date'],
            descope: row['Descope'],
            reasonOfExtraVisit: row['Reason of Extra Visit'],
            wccReceived80Percent: row['WCC Received 80%'],
            wccReceivedDate80Percent: row['WCC Received Date 80%'],
            wccReceived20Percent: row['WCC Received 20%'],
            wccReceivedDate20Percent: row['WCC Received Date 20%'],
            wccReceivedDate100Percent: row['WCC Received Date 100%'],
            survey: row['Survey'],
            finalPartnerSurvey: row['Final Partner Survey'],
